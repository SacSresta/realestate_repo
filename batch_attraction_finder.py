"""Compatibility wrapper for attractions.batch_attraction_finder

Keep using the original script path; this re-exports functions and CLI.
"""
from attractions.batch_attraction_finder import *  # noqa: F401,F403
#!/usr/bin/env python3
"""
Batch Attraction Finder Script

This script processes multiple properties from a CSV file containing latitude and longitude
coordinates and finds nearby attractions for each property.

Usage:
    python batch_attraction_finder.py --input geocoded_properties.csv --output results.csv
    
Input CSV should have columns: latitude, longitude
Optional columns: address, suburb, state, postcode (for identification)
"""
...
def process_properties_batch(input_file, output_file, radius=1000, limit=60, delay=3):
    """
    Process multiple properties from a CSV file and find attractions for each.
    
    Args:
        input_file: Path to input CSV file
        output_file: Path to output CSV file
        radius: Search radius in meters
        limit: Maximum attractions per property
    ...
    try:
        ...
        # Add predefined category columns
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing properties"):
            try:
                ...
                # Update the dataframe with predefined category results
                for category in ATTRACTION_CATEGORIES.keys():
...
def analyze_batch_results(df):
    """
    Analyze and summarize the batch processing results.
    
    Args:
        df: DataFrame with attraction analysis results
    """
    print(f"\n{'='*60}")
    ...
    # Properties with highest scores
    ...
    for idx, row in top_walkability.iterrows():
        ...
        print(f"  Score: {row['walkability_score']:.1f}, "
              f"Attractions: {row['total_attractions']}{address_info}")
